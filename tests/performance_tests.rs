...
async fn test_concurrent_proxy_queries() {
    ...
    match test_mode {
        TestMode::Real(_) => {
            // 真实服务测试：验证性能指标
            assert!(elapsed < Duration::from_secs(10), "并发请求应该在10秒内完成");
            println!("真实服务性能测试通过");
        },
        TestMode::Mock(_) => {
            // 模拟服务测试：应该有更好的性能
...
async fn test_rule_matching_performance() {
    ...
    match test_mode {
        TestMode::Real(_) => {
            // 真实服务测试：验证匹配性能
            assert!(avg_match_time < Duration::from_millis(1000), "平均匹配时间应该<1秒");
            println!("真实服务规则匹配性能测试通过");
        },
        TestMode::Mock(_) => {
            // 模拟服务测试：应该有更好的性能
            assert!(avg_match_time < Duration::from_millis(500), "模拟服务平均匹配时间应该<500ms");
            let success_count = match_results.iter().filter(|(_, success, _)| *success).count();
            assert!(success_count >= test_domains.len() / 2, "至少一半的匹配应该成功");
...
async fn test_monitoring_performance() {
    ...
    for i in 0..monitoring_cycles {
        ...
        // 获取性能统计
        let stats = monitor.get_performance_stats(Duration::from_secs(60));
        
        let cycle_elapsed = cycle_start.elapsed();
        println!("监控周期 {}: 状态获取成功={}, 耗时={:?}", i + 1, status_success, cycle_elapsed);
    ...
    match test_mode {
        ...
        TestMode::Mock(_) => {
            // 模拟服务测试：应该有更好的性能
            assert!(avg_cycle_time < Duration::from_secs(1), "模拟服务平均监控周期应该<1秒");
