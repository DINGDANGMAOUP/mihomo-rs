...
fn channel_parsing_accepts_expected_values() {
    assert_eq!("stable".parse::<Channel>().expect("stable parse"), Channel::Stable);
    assert_eq!("beta".parse::<Channel>().expect("beta parse"), Channel::Beta);
    assert_eq!(
        "nightly".parse::<Channel>().expect("nightly parse"),
        Channel::Nightly
    );
    assert_eq!("alpha".parse::<Channel>().expect("alpha parse"), Channel::Nightly);
    let err = "unknown"
        .parse::<Channel>()
