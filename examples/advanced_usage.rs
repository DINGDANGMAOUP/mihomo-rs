...
use mihomo_rs::{
    ...
    monitor::{Monitor, MonitorConfig, EventLevel, HealthStatus},
    proxy::ProxyManager,
    rules::RuleEngine,
    types::*,
...
async fn auto_proxy_selection(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let group_data: Vec<(String, ProxyGroup)> = groups.into_iter().map(|(k, v)| (k.clone(), v.clone())).collect();
    
    for (group_name, group) in group_data {
            if group.group_type == ProxyGroupType::Selector || group.group_type == ProxyGroupType::UrlTest {
                ...
                match proxy_manager.test_proxy_delay(proxy_name, Some("http://www.gstatic.com/generate_204"), Some(5000)).await {
                    Ok(delay) => {
                    println!("     {} 延迟: {}", proxy_name, format_utils::format_latency(delay.delay as u64));
            ...
            // 选择延迟最低的代理
            if let Some((best_proxy, best_delay)) = proxy_delays.iter().min_by_key(|(_, delay)| delay.delay) {
                println!("     最佳代理: {} (延迟: {})", best_proxy, format_utils::format_latency(best_delay.delay as u64));
...
/// 智能规则管理
async fn intelligent_rule_management(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, rule) in rules.iter().enumerate() {
        if let Err(e) = validation_utils::validate_rule_config(&rule.rule_type, &rule.payload, &rule.proxy) {
...
async fn advanced_monitoring(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match monitor.get_system_status().await {
        Ok(status) => {
            ...
            println!("     当前上传速度: {}", format_utils::format_speed(status.traffic.up));
            println!("     当前下载速度: {}", format_utils::format_speed(status.traffic.down));
...
async fn performance_optimization(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let proxy_names: Vec<_> = proxies.keys().take(5).map(|s| s.clone()).collect();
    ...
    for proxy_name in proxy_names {
        let result = proxy_manager.test_proxy_delay(&proxy_name, Some("http://www.gstatic.com/generate_204"), Some(3000)).await;
        results.push((proxy_name, result));
    }
    
    let elapsed = start_time.elapsed();
    println!("   并发测试完成，耗时: {}", time_utils::format_duration(elapsed));
    
    // 显示结果
    for (proxy_name, result) in results {
        match result {
            Ok(delay) => println!("     {} 延迟: {}", proxy_name, format_utils::format_latency(delay.delay as u64)),
...
async fn batch_operations(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 批量测试代理延迟
    let proxies = proxy_manager.get_proxies().await?;
    let proxy_names: Vec<_> = proxies.keys().take(10).map(|s| s.clone()).collect();
    
    println!("   批量测试 {} 个代理...", proxy_names.len());
    
    let mut test_results = HashMap::new();
    for proxy_name in &proxy_names {
        match client.test_proxy_delay(proxy_name, Some("http://www.gstatic.com/generate_204"), Some(5000)).await {
            Ok(delay) => { test_results.insert(proxy_name.clone(), Ok(delay)); }
            Err(e) => { test_results.insert(proxy_name.clone(), Err(e)); }
        }
    }
    ...
    if successful_tests > 0 {
        ...
        println!("     平均延迟: {}", format_utils::format_latency(avg_delay as u64));
    }
    
    // 自动选择最快的代理
    if let Some((fastest_proxy, _)) = test_results.iter().filter_map(|(name, result)| {
        result.as_ref().ok().map(|delay| (name.clone(), delay.clone()))
    }).min_by_key(|(_, delay)| delay.delay) {
        println!("   最快代理: {}", fastest_proxy);
        
        // 获取代理组并尝试切换
        let groups = proxy_manager.get_proxy_groups().await?;
        let group_names: Vec<_> = groups.iter()
            .filter(|(_, group)| group.all.contains(&fastest_proxy) && group.group_type == ProxyGroupType::Selector)
            .map(|(name, _)| name.clone())
            .collect();
        
        for group_name in group_names {
            match proxy_manager.switch_proxy(&group_name, &fastest_proxy).await {
                Ok(_) => println!("   已将组 '{}' 切换到最快代理: {}", group_name, &fastest_proxy),
                Err(e) => println!("   切换代理失败: {}", e),
            }
        }
    }
...
async fn automated_operations(client: &MihomoClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..3 { // 示例运行3次
        ...
        if let Ok(connections) = client.connections().await {
            let long_connections = connections.iter()
                .filter(|conn| {
                    // 模拟检查长时间连接
